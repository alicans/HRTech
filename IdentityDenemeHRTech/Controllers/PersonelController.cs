...
namespace HRTechProject.Controllers
{
    public class PersonelController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if(!personel.SifreDegistiMi && personel.Email != "admin@example.com")
            {
        ...
        public async Task<IActionResult> SifreDegistir(ChangePasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var changePasswordResult = await _userManager.ChangePasswordAsync(user, model.EskiSifre, model.Sifre);

                if (changePasswordResult.Succeeded)
                {
                    user.SifreDegistiMi = true;
        ...
        public async Task<IActionResult> Izin(int page = 1)
        {
            ...
            var pagedItemList = izinler.Skip((page - 1) * 15).Take(15);

            ViewBag.PageNumber = page;
            ViewBag.TotalPages = Math.Ceiling((double)izinler.Count / 15);

            DateTime sonTamSene = personel.IseGirisTarihi.Value;           

            if (personel.IzinHakki > 14)
            {                
                while (true)
                {
                    sonTamSene = sonTamSene.AddYears(1);
                    if(personel.Aktiflik == true)
                    {
                        if(sonTamSene > DateTime.Now)
                        {
                            sonTamSene = sonTamSene.AddYears(-1);
                            break;
                        }                    
                    ...
                    {
                        if (sonTamSene > personel.IstenCikisTarihi)
                        {
                            sonTamSene = sonTamSene.AddYears(-1);
                            break;
                        }
            ...
            int kullanilanIzin = _db.Izinler
                .Where(x => x.ApplicationUserId == userId)
                .Where(x => x.BitisTarihi < sonTamSene && x.Turu == IzinTuru.Diger && x.Durumu == IzinDurumu.Onay)
                .ToList()
                .Sum(x => x.GunSayisi);

            int sonDonemKullanilanIzin = _db.Izinler
                .Where(x => x.ApplicationUserId == userId)
                .Where(x => x.BaslangicTarihi > sonTamSene && x.Turu == IzinTuru.Diger && x.Durumu == IzinDurumu.Onay)
                .ToList()
                .Sum(x => x.GunSayisi);

            int gecenSenelerdenDevir = personel.IzinHakki - 14 - kullanilanIzin;            

            int toplamNetIzin = 14 + Math.Min(14, gecenSenelerdenDevir);

            if (personel.IseGirisTarihi.Value.AddYears(1) > DateTime.Now)
            {
                gecenSenelerdenDevir += 14;
            }

            ViewBag.GecenSenelerdenDevir = gecenSenelerdenDevir;
            ViewBag.ToplamNetIzin = toplamNetIzin; 
            ViewBag.sonDonemKullanilanIzin = sonDonemKullanilanIzin;
        ...
        public async Task<IActionResult> IzinEkle(Izin yeniIzin)
        {
            ...
            {                
                ...
                {                    
                    _toast.AddErrorToastMessage("Başlangıç tarihi bitiş tarihinden sonra olamaz!", new ToastrOptions { Title = "İşlem Başarısız" });
                    return View(yeniIzin);
                }
                string errorMessage = "";
                if (!yeniIzin.GunSayisiKisitlamasi(out errorMessage))
                {
                    _toast.AddErrorToastMessage($"{errorMessage}", new ToastrOptions { Title = "İşlem Başarısız" });
                ...
                if(personel.IzinHakki < yeniIzin.GunSayisi && yeniIzin.Turu == IzinTuru.Diger)
                {
                    _toast.AddErrorToastMessage("Belirtilen izin türü için izin hakkı yetersiz!", new ToastrOptions { Title = "İşlem Başarısız" });                    
                    return View(yeniIzin);
                }

                if (yeniIzin.Turu == ApplicationCore.Enums.IzinTuru.Babalik && personel.Cinsiyet == ApplicationCore.Enums.Cinsiyet.Kadin)
                {
                    _toast.AddErrorToastMessage("Kadın personel babalık iznini seçemez!", new ToastrOptions { Title = "İşlem Başarısız" });                    
                    return View(yeniIzin);
                }
                if (yeniIzin.Turu == ApplicationCore.Enums.IzinTuru.Hamilelik && personel.Cinsiyet == ApplicationCore.Enums.Cinsiyet.Erkek)
                {
                    _toast.AddErrorToastMessage("Erkek personel hamilelik iznini seçemez!", new ToastrOptions { Title = "İşlem Başarısız" });                    
                    return View(yeniIzin);
                }
